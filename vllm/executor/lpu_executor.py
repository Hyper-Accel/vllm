...
class LPUExecutor(ExecutorBase):
    ...
    def _init_executor(self) -> None: #HJ: why not __init__ ?
        ...
        assert not self.speculative_config, (
            "Speculative decoding is not yet supported for LPU backend")
        if self.model_config.dtype in (torch.float16, torch.float32):
            logger.warning(
                "The LPU backend currently does not support %s. "
    ...
    def _create_worker(
        ...
        from vllm.worker.lpu_worker import LPUWorker

        worker = LPUWorker(**self._get_worker_kwargs(local_rank, rank,
    ...
    def initialize_cache( #HJ: we are not use cache
        ...
        logger.info("# LPU blocks: %d, # CPU blocks: %d", num_gpu_blocks,
                    num_cpu_blocks)
    ...
    def add_lora(self, lora_request: LoRARequest) -> bool:
        raise NotImplementedError(
            "LoRA is currently not supported by the LPU backend.")
    ...
    def add_prompt_adapter(self, prompt_adapter_request) -> bool:
        raise NotImplementedError(
            "Soft prompt is currently not supported by the LPU backend.")
    ...
    def check_health(self) -> None:
        # LPUExecutor will always be healthy as long as it's running.
        return


class LPUExecutorAsync(LPUExecutor, ExecutorAsyncBase):
